...
interface Function {
    myCall<T extends object, R>(context: T, ...args: any[]): R,
    myApply<T extends object, R>(context: T, args: any[]): R,
    myBind<T extends object>(context: T, ...args: any[]): Function,
}

Function.prototype.myCall = function <T extends object, R>(context: T, ...args: any[]): R {
    // 可能不传参，不传时为window
...
function myNew<T extends { new(...args: any[]): any }>(constructor: T, ...args: any[]) {
...
function deepClone<T extends object | any[]>(obj: T, map = new WeakMap<object, any>()): T {
    ...
    if (isArray) {
        obj.forEach((val, index) => {
            (result as any[])[index] = deepClone(val, map)
        })
    } else {
        Object.keys(obj).forEach((key) => {
            (result as Record<string, any>)[key] = deepClone(obj[key], map)
        })
    }
